...
DT&BGV_Fuse_Update
    ...
    Sleep    2s
    Log To Console    *********************Initiating Fuseupdate*********************
    Append To File    ${Log}    *********************Initiating Fuseupdate************************\n
    TRY
    ...
    IF    ${F_Login_2} == True
        # TRY
            ...
            IF    "${Alldatas}[tax_term]" == "1099"
                TRY
                    Click Button    //*[@id="manage_modal"]/div/div/div[1]/button
                EXCEPT
                    Log To Console    Nothing is Open
                END
                Wait Until Element Is Visible    //*[@id="frm_filter_data"]/div[1]/span/label/div    10s
            ...
            IF    "${AllFullname}" == "False"
                ...
                ${Body}=    Set Variable
                ...    Technician not found, Bot didn't updated Spectrum DT&BGV Initiation details for ${Alldatas}[tech_first_name] ${Alldatas}[tech_last_name] in Fuse. Kindly update manually.
            ...
            Sleep    1s
            Append To File    ${Log}    Drug test Updated in Fuse.\n
            Log To Console    Drug test Updated
            # ************Background************
            ...
            TRY
                Wait Until Element Is Visible    //*[@id="rnote"]    3s
                Input Text When Element Is Visible    //*[@id="rnote"]    DT/BGV is Initiated in Crimshield
                Sleep    2s
                Click Element If Visible    //*[@id="update_recruit_data_form"]/div[1]/table/tbody/tr[11]/td[2]/input
                Sleep    1s
                Log To Console    Notes updated in first try
                Append To File    ${Log}    Notes updated in first try.\n
            EXCEPT
                Log To Console    Unable to enter the notes
                 Append To File    ${Log}    Notes not updated in first try.\n
            END
            IF    "${Alldatas}[tax_term]" == "w2"
                # Submit
                ...
                END
                # Notes Update - w2
                Wait Until Element Is Visible    //*[@id="frm_filter_data"]/div[1]/span/label/div    5s
                Click Element If Visible    //*[@id="frm_filter_data"]/div[1]/span/label/div
                Sleep    10s
                Wait Until Element Is Visible    //*[@id="exampleImport_filter"]/label/input    10s
                Input Text    //*[@id="exampleImport_filter"]/label/input    ${db_full_name}
                ...
                FOR    ${index}    IN RANGE    1    ${row_count+1}
                    TRY
                        ...
                        IF    '${full_name}' == '${db_full_name}'
                            ${row_index}=    Set Variable    ${index}
                            Wait Until Element Is Visible
                            ...    //table[@id='exampleImport']/tbody/tr[${row_index}]/td[1]/input[1]
                            ...    5s
                            ${element}=    Get WebElement
                            ...    //table[@id='exampleImport']/tbody/tr[${row_index}]/td[1]/input[1]
                            ${value}=    Get Element Attribute    ${element}    value
                            IF    '${value}' == 'Recruit Info'
                                Click Element If Visible
                                ...    //table[@id='exampleImport']/tbody/tr[${row_index}]/td[1]/input[1]
                                ${Fullname}=    Set Variable    True
                                BREAK
                ...
                IF    "${AllFullname}" == "False"
                    ...
                    END
                ELSE
                    Wait Until Element Is Visible    //*[@id="rnote"]    10s
                    Input Text When Element Is Visible    //*[@id="rnote"]    DT/BGV is Initiated in Crimshield
                    Sleep    2s
                    Click Element If Visible    //*[@id="update_recruit_data_form"]/div[1]/table/tbody/tr[11]/td[2]/input
                    Sleep    1s
                    Click Element If Visible    //button[@type='button' and contains(@class, 'close') and @onclick="$('#recruit_modal').modal('hide');"]
                    Append To file    ${Log}    Fuse note updated.\n
                    Log To Console    Fuse note updated.
                END
            ELSE
                # Notes Update - 1099
                Wait Until Element Is Visible    //*[@id="Notedb"]    5s
                Input Text When Element Is Visible    //*[@id="Notedb"]    DT/BGV is Initiated in Crimshield
                Sleep    1s
                Click Element If Visible
    ...
    ELSE
        ...
        END
        ${Body}=    Set Variable
        ...    Fuse not found, Bot didn't updated Spectrum DT&BGV Initiation details for ${Alldatas}[tech_first_name] ${Alldatas}[tech_last_name] in Fuse. Kindly update manually.
        ${Attachment}=    Set Variable    ${doc}
        ${Mailsent}=    Run Keyword And Return Status
        ...    Sendmail
        ...    ${Recepients}
