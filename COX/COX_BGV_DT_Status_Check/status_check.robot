            ...
            IF    "${TechStatus}" == "Complete"
                ...
                IF    ${status_Pdf} == True
                    ...
                    IF    ${ProfileStatus} == True
                        ...
                        Append To file    ${DTLog}    bgv_completed_date=${date}.\n
            ...
            ELSE
                ...
                IF    "${TechStatus}" == "" or "${TechStatus}" == "None"
                    IF    "${Alldatas}[status_completed_mail_sent]" != "Remainder_Sent"
                        ...
                        END
                        ${created_date}=    Convert Date    ${created_date}    result_format=%Y-%m-%d    date_format=%m/%d/%Y
                        ${check_date}=    Add Time To Date    ${created_date}    ${DAYS_TO_WAIT} days     result_format=%Y-%m-%d
                        Log To Console    Check date (3 days after created date) = ${check_date}
                        Append To File    ${DTLog}    Check date (3 days after created date) = ${check_date}.\n
                        IF    '${current_date}' >= '${check_date}'
                            Log To Console    ALERT: 4 days have passed and status is still empty!
                            ${Still_Empty}=    Set Variable    True
                        ELSE
                            Log To Console    No alert needed. Less than 4 days have passed.
                            ${Still_Empty}=    Set Variable    False
                        END
                        IF  "${Still_Empty}" == "True"
                            ${Body}=    Set Variable    
                            ...    Please be reminded that tech "${first_name}${last_name}" has an order number is (${order_number}) and order date is (${order_date}), four days passed still status is empty.    
                            ${Attachment}=    Set Variable    ${doc}
                            ${Recepients}=    set variable    ${credentials}[cox_team]
                            ${CC}=    set variable    ${credentials}[Recipient]
                            IF    "${Alldatas}[tax_term]" == "1099"
                            ...
                            END
                        ELSE
                            Log to console    Less than 4 days
                            Append To File    ${DTLog}    Less than 4 days.\n
                        END
                    END
                ELSE
